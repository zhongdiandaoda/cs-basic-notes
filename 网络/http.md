...
HTTP 可以通过和 SSL（Secure Socket layer，安全套接层），或 TLS（Transport Layer Security，安全层传输协议）组合使用，加密HTTP的通信内容。首先通过SSL建立安全通信线路，再通过HTTP协议通信。

HTTP 与 SSL 的组合被称为HTTPS，即 HTTP Secure 或 HTTP over SSL。
